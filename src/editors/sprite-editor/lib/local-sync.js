...
const storage = (function getStorageAdapter() {
  ...
  try {
    ...
    console.error(err)
    console.info(
      'LocalSync: localStorage was not available, falling back to in-memory storage.',
    )
    adapter = memoryStorage
  }
...
class LocalSync {
   ...
   */
  _parseKey(fullKey) {
    const fullPrefix = this._fullKey('REMOVE_ME').replace('REMOVE_ME', '')
  ...
  _validatePrefix(prefix) {
    if (typeof prefix !== 'string')
      throw new Error('LocalSync "prefix" must be a string.')
    if (prefix.indexOf(this._separator) !== -1) {
      throw new Error(
        `LocalSync "prefix" cannot contain the separator "${this._separator}".`,
      )
    }
    return prefix
  }
  ...
  _validateSeparator(separator) {
    if (typeof separator !== 'string')
      throw new Error('LocalSync "separator" must be a string.')
    if (separator.length !== 1)
      throw new Error('LocalSync "separator" must be a single character.')
    return separator
  }
  ...
  _validateBucket(bucket) {
    if (typeof bucket !== 'string')
      throw new Error('LocalSync "bucket" must be a string.')
    if (bucket.indexOf(' ') !== -1)
      throw new Error('LocalSync "bucket" cannot contain spaces.')
    if (bucket.indexOf(this._separator) !== -1) {
      throw new Error(
        `LocalSync "bucket" cannot contain the separator "${this._separator}".`,
      )
    }
    return bucket
  }
  ...
  _validateKey(key) {
    if (typeof key !== 'string')
      throw new Error('LocalSync "key" parameter must be a string.')
    if (key.indexOf(this._separator) !== -1) {
      throw new Error(
        `LocalSync "key" cannot contain the separator "${this._separator}".`,
      )
    }
    return key
  }
  ...
  /**
   * Get a first value from the current bucket.
   * @returns {*} The stored value at the specified `key`.
   */
  getFirst() {
